...
# במקרה שה-import לא עובד כראוי, נגדיר את הקבועים ידנית
SIDE_BUY = 'BUY'
SIDE_SELL = 'SELL'
...
PORTFOLIO_USD = 1000
# עדכון רשימת המטבעות - הסרת מטבעות בעייתיים
symbols = ['BTCUSDT', 'ETHUSDT', 'SOLUSDT', 'BNBUSDT', 'APTUSDT']
open_positions = {}
...
def setup_order_status_monitor(symbol):
    ...
    try:
        ...
        # שמירת המידע בקובץ
        try:
            with open(f"{symbol}_position_data.json", "w") as f:
                json.dump(position_data, f)
        except Exception as e:
            print(f"שגיאה בשמירת נתוני פוזיציה: {e}")
...
def monitor_position_status(symbol):
    ...
    try:
        # טעינת נתוני הפוזיציה
        try:
            with open(f"{symbol}_position_data.json", "r") as f:
                position_data = json.load(f)
        except:
            print(f"לא ניתן לטעון נתוני פוזיציה עבור {symbol}")
            return
        ...
        while True:
            ...
            if not position_exists:
                ...
                # מחיקת קובץ המעקב
                try:
                    os.remove(f"{symbol}_position_data.json")
                except:
                    pass
...
def log_trade(symbol, direction, score, valid_until):
    """תיעוד עסקאות לקובץ לוג"""
    try:
        with open("trade_log.txt", "a") as log_file:
            log_file.write(f"{datetime.now()} | {symbol} | {direction} | Score: {score} | Valid Until: {valid_until}\n")
    except Exception as e:
        print(f"שגיאה בכתיבה לקובץ לוג: {e}")
...
if __name__ == "__main__":
    run_bot()# TOM_AI - מערכת מסחר לייב בקריפטו מבוססת אינדיקטור TOM
